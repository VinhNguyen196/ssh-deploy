            ...
            steps {
               script {
                   withCredentials([usernamePassword(credentialsId: 'SSH-Deploy', passwordVariable: 'pass', usernameVariable: 'user')]) {
                        ...
                        def temp_str = sshCommand remote: remote, failOnError: false, command: "\$(docker images | grep $DOCKER_IMAGE)"
                        ...
                        // sshCommand remote: remote, command: "sudo sed -i 's\\export DOCKER_TAG=.*\\export DOCKER_TAG=\"$DOCKER_TAG\"\\g' ~/.bashrc"
